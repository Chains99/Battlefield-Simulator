...
fraction0.add_soldiers([s1, s2])
fraction1.add_soldiers([s3, s4])
...
sim = SimulationManager([fraction0, fraction1], weather, sim_map=mapa, heuristics=heur, max_depth=5)
initial_state = sim.build_initial_state(sim.fractions, sim.ab.am, soldier_pos)

btf = BattleField(sim, console=None)
